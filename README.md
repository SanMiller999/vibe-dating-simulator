...
## Notification-to-chat routing + NPC archetypes — v1.12.3
...
## NPC archetype expansion — v1.12.3
...
## Expectation / Revelation — v1.12.3
...
## AI-initiated first messages — v1.12.3
...
## Notification dismissal — v1.12.3
...
## Profile readability cleanup — v1.12.3
...
## Complete NPC profiles — v1.12.3

- Removed the player-page block «Как вас увидят в знакомствах» completely.
- All dating participants, including the built-in profiles and newly generated NPCs, now normalize to the same public-profile field structure as the player's profile.
- Dating cards include an «Открыть профиль» action.
- Static NPC profiles receive a compatible AI/profile data layer for future Character Brain integration.


## Dating flow correction — v1.12.3

- «Знакомства» is strictly like/dislike; no profile-opening action and no automatic chat.
- A mutual like creates a `match` activity in Notifications.
- Match notifications can be read, deleted with `×`, or use the actor name to open the public profile.
- Only a `chat_message` notification opens the chat directly from its notification body.
- Starting a conversation from the public profile opens the empty persistent chat.
- NPC profiles remain available in Notifications/Chats/explicit profile navigation and keep the same public-profile schema as the player.
